...
export type PmsParticleHistoryPeak = Record<PmsParticleKey, number>;

export function computeParticleHistoryPeak(records: Array<Record<string, unknown>>): PmsParticleHistoryPeak {
  const peak: PmsParticleHistoryPeak = {
    '0p3': 0,
    '0p5': 0,
    '1p0': 0,
    '2p5': 0,
  ...
  for (const record of records) {
    ...
    for (const item of PARTICLE_ROWS) {
      const value = readValue(source, item.key);
      if (value > peak[item.key]) {
        peak[item.key] = value;
      }
    }
  }
...
export function buildParticleRows(record: Record<string, unknown> | null, historyPeak: PmsParticleHistoryPeak | null = null): PmsParticleRow[] {
  const source = extractParticleSource(record);
  const values = PARTICLE_ROWS.map((row) => (source ? readValue(source, row.key) : 0));
  const samplePeak = Math.max(...values, 1);

  return PARTICLE_ROWS.map((row, index) => {
    const value = values[index];
    const peak = Math.max(historyPeak?.[row.key] ?? 0, value);
